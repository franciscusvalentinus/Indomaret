...
<div class="container">
    ...
    <div class="modal fade" id="importExcel" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <form method="post" action="/productuser/import_excel" enctype="multipart/form-data">
    ...
    <table class='table table-bordered'>
        ...
        <tbody>
        @php $i=1 @endphp
        @foreach($productuser as $pu)
            <tr>
                <td>{{ $i++ }}</td>
                <td>{{$pu->user_id}}</td>
                <td>{{$pu->produk_id}}</td>
                <td>{{$pu->kode_produk}}</td>
                <td>{{$pu->nama_produk}}</td>
                <td>{{$pu->harga_produk}}</td>
                <td>{{$pu->harga_jual}}</td>
            </tr>
        @endforeach
        </tbody>
