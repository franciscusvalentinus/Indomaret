            ...
            <section class="panel">
                <div class="panel-body progress-panel">
                    <div class="row">
                        <div class="col-lg-8 task-progress pull-left">
                            <h1>Laporan Transaksi Per Produk</h1>
                        </div>
                    </div>
                </div>
                <div class="panel-body">
                    <div class="form-group">
                        <div class="row">
                            <label class="col-md-2 control-label">Pilih Cabang</label>

                            <label class="col-md-2 control-label">Pilih Toko</label>

                            <label class="col-md-2 control-label">Pilih PLU</label>

{{--                            ini buat space kosong aja--}}
                            <label class="col-md-2 control-label"></label>

                            <label class="col-md-2 control-label">Pilih Tanggal Mulai</label>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-2">
                                <select name="scopeplu" id="scopeplu" class="selectpicker form-control" data-live-search="true" onchange="">
                                    <option style='font-size: 12px;' value='%' selected>SEMUA PLU???</option>
                                </select>
                            </div>
                            {{--                            ini buat space kosong aja--}}
                            <div class="col-md-2">
                            ...
                            <div class="col-md-2">
                                <input type="date" id="month_start" name="month_start" max="{{\Carbon\Carbon::now()->format('Y-m-d')}}"
                                       min="2018-03-01" value={{\Carbon\Carbon::now()->format('Y-m-d')}}>
                            </div>
                            <div class="col-md-2">
                                <input type="date" id="month_end" name="month_end" max="{{\Carbon\Carbon::now()->format('Y-m-d')}}"
                                       min="2018-03-01" value={{\Carbon\Carbon::now()->format('Y-m-d')}}>
                        ...
                        <div class="row">
                            <div class="col-md-6" >
                                {{--                                    <select name="scopejenis" id="scopejenis" class="selectpicker form-control">--}}
                                    ...
                                    {{--                                        DIBAWAH INI TIDAK DI MINTA UNTUK DI TAMPILKAN--}}
                ...
                <form id="exportform" action="export_isaku_payment" method="post" target="_blank">
...
<script>
    ...
    function changePlu() {
        $.ajax({
            ...
            success:function(response)
            {
                $('#scopeplu').empty().append(response);
                $('#scopeplu').selectpicker('refresh');
                $('#exportButton').css("display","none");
            }
        });
    ...
    {
        var cabang = $('#scopecabang').val();
        var toko = $('#scopetoko').val();
        var month_start = $('#month_start').val();
        var month_end = $('#month_end').val();
        var plu = $('#scopeplu').val();

        console.log(month_start);
        console.log(month_end);

        if(plu == '%'){
            alert('Anda harus pilih plu!');
        } else {
            $.ajax({
                url: 'export_laporan_transaksi_per_plu',
                type: 'POST',
                data: {
                    'cabang': cabang,
                    'toko': toko,
                ...
                success: function (response) {
                    ...
                    a.remove();
{{--                    {{ csrf_token() }}--}}
                    //                     window.location = 'export_spd_std_apc'
                    // console.log('success van');
